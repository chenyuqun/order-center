...
public class TradeServiceOrderCreateBizzImpl implements TradeServiceOrderCreateBizz {
    ...
    public void validateTradeServiceOrderCreateParam(TradeServiceOrderCreateParam param)
        ...
        if (param.getChannel() == null) {
            throw new IllegalParamterException("channel is not null");
        }
        ...
        if (param.getBusinessId() == null || param.getBusinessId() != 0) {
            throw new IllegalParamterException("businessId is not null");
        }
        if (param.getCustomerId() == null || param.getCustomerId() != 0) {
            throw new IllegalParamterException("customerId is not null");
        }
        // 非中国需要填写
        if (param.getCustomerDestId() != Country.ZH.getValue() && param.getCustomerDestId() != Country.HK.getValue()
                && param.getCustomerDestId() != Country.TAIWAN.getValue()) {
    ...
    public ResponseResult createServiceOrder(TradeServiceOrderCreateParam param) throws ZZKServiceException {
        ...
        // 预定服务参数
        AdditionalServiceParam additionalServiceParam = param.getAdditionalServiceParam();
        AdditionalService additionalService = additionalServiceService.queryByServiceId(additionalServiceParam
                .getServiceId());
        ...
        tradeServiceOrder.setCustomerId(param.getCustomerId());
        tradeServiceOrder.setBusinessId(param.getBusinessId());
        tradeServiceOrder.setChannel(param.getChannel());
